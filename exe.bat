...
start "Backend - FastAPI" cmd /k "call ocpct\Scripts\activate & uvicorn main:app --reload"
...
start "Receptor - UDP" cmd /k "call ocpct\Scripts\activate & python receptor.py"

:: 3. Inicia o Simulador UDP em uma nova janela
echo [3/4] Iniciando o Simulador de Telemetria...
start "Simulador - UDP" cmd /k "call ocpct\Scripts\activate & python simulador.py"

:: 4. Inicia o Dashboard (Streamlit) em uma nova janela
echo [4/4] Abrindo o Painel de Controle...
start "Frontend - Streamlit" cmd /k "call ocpct\Scripts\activate & streamlit run dashboard.py"
